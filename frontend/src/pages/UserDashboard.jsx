        ...
        <div className='flex flex-row'>
          <div className='flex flex-col border-r-2 border-sky-700 w-1/5'>
            <ul className='pt-1 pl-4 gap-1 flex flex-col'>
              ...
              <li
                className={`hover:bg-slate-100 p-2 pl-3 ${selected === 'appliedServices' && selectStyle}`}
                onClick={() => { setContent('appliedServices'); setSelected('appliedServices'); }}
              >
                Applied Services
              </li>
              <li
                className={`hover:bg-slate-100 p-2 pl-3 ${selected === 'currentServices' && selectStyle}`}
                onClick={() => { setContent('currentServices'); setSelected('currentServices'); }}
              >
                Current Services
              </li>
              <li
                className={`hover:bg-slate-100 p-2 pl-3 ${selected === 'history' && selectStyle}`}
                onClick={() => { setContent('history'); setSelected('history'); }}
              >
                History
              </li>
            </ul>
          ...
          <div className='h-[40rem] w-4/5 flex flex-col items-center overflow-y-scroll'>
            <div className={`transition-opacity duration-300 ${transitioning ? 'opacity-0' : 'opacity-100'}`}>
              {currentContent === 'appliedServices' && <AppliedServices user={user} />}
              {currentContent === 'currentServices' && <CurrentServices user={user} />}
              {currentContent === 'history' && <History user={user} />}
            </div>
          </div>
        </div>
        <div className='p-10'>
